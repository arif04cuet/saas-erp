function toggleDebitCreditAmountField(index, value) {
    let debitPattern = "hm_journal_entries[" + index + "][debit_amount]";
    let creditPattern = "hm_journal_entries[" + index + "][credit_amount]";
    if (value == 'receive') {
        // its receipt - credit
...
function initAllRepeater(className) {
    $(className).repeater({
        ...
        show: function () {
            ...
            let trainingElement = $('.select-fiscal-year ');
            if (!trainingElement.val()) {
                return;
            }
            let codeSelectElement = $('.select-hostel-section')
                .not((index, element) => {
                    if ($(element).val())
                        return true;
                    else
            ...
            filterSectionsByFiscalYear(getBudgetKeys($('.select-fiscal-year')), codeSelectElement);
            $(this).slideDown();
        },
...
/**
 * get all the related budget keys for a fiscal year
 * @param obj
 * @return {string[]}
 */
...
function getMaxValues(obj) {
    ...
    filterSectionsByFiscalYear(keys, $('.select-hostel-section'));
}

function filterSectionsByFiscalYear(keys, codeSelectElement) {
    let allKeys = Object.keys(hmBudgetSections);
    ...
    difference.forEach(element => {
        codeSelectElement.append(
            '<option value="' + element + '">' + hmBudgetSections[element] + '</option>'
        )
    });
}
