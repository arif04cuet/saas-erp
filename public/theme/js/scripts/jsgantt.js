let JSGantt; if (!JSGantt) JSGantt = {};
...
JSGantt.TaskItem = function(pID, pName, pStart, pEnd, pColor, pLink, pMile, pRes, pComp, pGroup, pParent, pOpen, pDepend, pCaption)
{
      ...
      let vID    = pID;
      let vName  = pName;
      let vStart = JSGantt.parseDateStr(pStart,g.getDateInputFormat());
      let vEnd   = JSGantt.parseDateStr(pEnd,g.getDateInputFormat());
      let vColor = pColor;
      let vLink  = pLink;
      let vMile  = pMile;
      let vRes   = pRes;
...
JSGantt.GanttChart =  function(pGanttVar, pDiv, pFormat)
{
      ...
      let vGanttVar = pGanttVar;
      let vDiv      = pDiv;
      let vFormat   = pFormat;
      let vShowRes  = 1;
      let vShowDur  = 1;
      let vShowComp = 1;
      let vShowStartDate = 1;
      let vShowEndDate = 1;
      let vDateInputFormat = "mm/dd/yyyy";
      let vDateDisplayFormat = "mm/dd/yy";
	  let vNumUnits  = 0;
      let vCaptionType;
      let vDepId = 1;
      let vTaskList     = new Array();
	  let vFormatArr	= new Array("day","week","month","quarter");
      let vQuarterArr   = new Array(1,1,1,2,2,2,3,3,3,4,4,4);
      let vMonthDaysArr = new Array(31,28,31,30,31,30,31,31,30,31,30,31);
      let vMonthArr     = new Array("January","February","March","April","May","June","July","August","September","October","November","December");
   ...
   this.Draw = function()
   {
      ...
      if(vTaskList.length > 0)
      {
         ...
         vLeftTable =
            ...
            '  <td style="text-align: center; font-size: 12px"><b>SL. NO.</b></td>' +
            '  <TD style="WIDTH: ' + vNameWidth + 'px; HEIGHT: 17px; text-align: justify"><b>Name of the Courses</b></TD>';



         vLeftTable +=
            '<TR style="HEIGHT: 20px">' +
            '  <TD style="BORDER-TOP: #efefef 1px solid; border-right: #efefef 1px solid; WIDTH: 400px; HEIGHT: 20px; font-size: 12px; text-align: center"> <b>A.</b></TD>' +
            '  <TD style="BORDER-TOP: #efefef 1px solid; WIDTH: ' + vNameWidth + 'px; HEIGHT: 20px; text-align: justify"><b>National Level Courses</b></TD>' ;
         ...
         vLeftTable += '</TR>';
            ...
            for(i = 0; i < vTaskList.length; i++)
            {
               ...
               vID = vTaskList[i].getID();
                 ...
                 vLeftTable += '<TR id=child_' + vID + ' bgcolor=#' + vBGColor + ' onMouseover=g.mouseOver(this,' + vID + ',"left","' + vRowType + '") onMouseout=g.mouseOut(this,' + vID + ',"left","' + vRowType + '")>' ;
			      ...
			      vLeftTable +=
                  '  <TD class=gdatehead style="WIDTH: 15px; HEIGHT: 20px; BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid;text-align: center">'+ vID +'</TD>' +
                  '  <TD class=gname style="WIDTH: ' + vNameWidth + 'px; HEIGHT: 20px; BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; text-align: justify;" nowrap><NOBR><span style="color: #aaaaaa">';
            ...
            '<TABLE style="width: ' + vChartWidth + 'px;margin-bottom: 18px;" cellSpacing=0 cellPadding=0 border=0>' +
