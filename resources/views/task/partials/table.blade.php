    ...
    <div class="card-header">
        ...
        <div class="heading-elements">
            <ul class="list-inline mb-0">
                ...
                <li>
{{--                    @if(isset($multi_parameter))--}}
{{--                        <a href="{{ route($module . '-tasks.create', $multi_parameter) }}"--}}
{{--                           class="btn btn-sm btn-primary"><i class="ft ft-plus"></i> @lang('task.add_task')</a>--}}
{{--                    @else--}}
                        <a href="{{ route($module . '-tasks.create', $taskable->id) }}"
                           class="btn btn-sm btn-primary"><i class="ft ft-plus"></i> @lang('task.add_task')</a>
{{--                    @endif--}}
                </li>
                @endif
            ...
            <table class="table task-table table-bordered table-striped">
                ...
                @foreach($taskable->tasks as $task)
                    ...
                    @if(isset($multi_parameter))
                        ...
                        <td>
                            <a href="{{ route($module . '-tasks.show', [$project->id, $taskable->id, $task->id]) }}">{{ $task->name }}</a>
                        </td>
                        <td class="text-center">
                            @if (isset($task->actual_start_time))
                                {{ \Carbon\Carbon::parse($task->actual_start_time)->format('d/m/Y') }}
                            @else
                                {{ Form::open(['route' => [$module . '-tasks.time', $project->id, $taskable->id, $task->id], 'method' => 'PUT', 'style' => 'display: inline']) }}
                        ...
                        @if(Auth::user()->hasAnyRole('ROLE_RESEARCHER') || Auth::user()->hasAnyRole('ROLE_PROJECT_DIRECTOR'))
                        <td class="text-center">
                            {{ Form::open(['route' => [$module . '-tasks.destroy', $project->id, $taskable->id, $task->id], 'method' => 'DELETE', 'style' => 'display: inline']) }}
