...
@section('content')
    ...
    @php
        $initialDeductionData = [];

        if($employeeLumpSum->deductions->count())
        {
             $initialDeductionData = $employeeLumpSum->deductions->map(function ($deduction) {
                return [
                    'pension_deduction_id' => $deduction->pensionDeduction->id,
...
@push('page-js')
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            let deductionForm = $('.deduction-repeater'),
                $initialDeductionData = @json($initialDeductionData, JSON_UNESCAPED_UNICODE);
            let deductionFormRepeater = deductionForm.repeater({
                show: function () {
                    $(this).slideDown();
                    deductionForm.find('select').next('.select2-container').remove();
                    deductionForm.find('select').select2({
            ...
            if ($initialDeductionData.length) {
                deductionFormRepeater.setList($initialDeductionData);
            }
        });
