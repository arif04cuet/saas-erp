            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title"><i class="ft-tag black"></i> {{ trans('labels.create') }}</h4>
                        <a class="heading-elements-toggle"><i class="la la-ellipsis-h font-medium-3"></i></a>
                    </div>
                    <div class="card-content collapse show">
                        <div class="card-body">
                            @include('hm::accounts.journal-entry.form.form')
                        </div>
                    </div>
                </div>
            </div>
...
@push('page-js')
    ...
    <!-- custom -->
    <script src="{{ asset('js/hm-accounts/journal-entry.js') }}"></script>
    <script>
        const placeholder = @json(trans('labels.select'));
        const allBudgetMaxValues = @json($maxBudgetValues);
        // console.log(allBudgetMaxValues);
        const hmBudgetSections = @json($hostelBudgetSections);
        let budgetMaxValues;
        $(document).ready(function () {
            initAllRepeater('.hm-journal-entry-repeater');
            validateForm('.hm-journal-entry-form');
        ...
        function addMaxValidationToCredit(element) {
            ...
            var debitPattern = "hm_journal_entries[" + index + "][debit_amount]";
