...
class ProjectActivityService
{
    ...
    public function getActivityForDropdown(
        Closure $implementedValue = null,
        Closure $implementedKey = null,
        array $query = null,
        $isEmptyOption = false
    ) {
        $activity = $query ? $this->actionRepository->findBy($query) : $this->actionRepository->findAll();

        return DropDownDataFormatter::getFormattedDataForDropdown(
            $activity,
            $implementedKey,
            $implementedValue ?: function ($activity) {
