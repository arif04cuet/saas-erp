            ...
            <table class="table task-table table-bordered table-striped">
                ...
                @foreach($taskable->tasks as $task)
                    ...
                    @endif
                        ...
                        <td>
                            <a href="{{ route('pms-activity-tasks.show', [$project->id, $taskable->id, $task->id]) }}">{{ $task->name }}</a>
                        </td>
                        <td class="text-center">
                            @if (isset($task->actual_start_time))
                                {{ \Carbon\Carbon::parse($task->actual_start_time)->format('d/m/Y') }}
                            @else
                                {{ Form::open(['route' => ['pms-activity-tasks.time', $project->id, $taskable->id, $task->id], 'method' => 'PUT', 'style' => 'display: inline']) }}
                        ...
                        @if(Auth::user()->hasAnyRole('ROLE_RESEARCHER') || Auth::user()->hasAnyRole('ROLE_PROJECT_DIRECTOR'))
                        <td class="text-center">
                            {{ Form::open(['route' => ['pms-activity-tasks.destroy', $project->id, $taskable->id, $task->id], 'method' => 'DELETE', 'style' => 'display: inline']) }}
