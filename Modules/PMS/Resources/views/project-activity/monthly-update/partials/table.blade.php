    ...
    <div class="card-header">
        ...
        <div class="heading-elements">
            <ul class="list-inline mb-0">
                <li><a href="{{ route($module . '-monthly-updates.create', [$project->id, $monthlyUpdatable->id]) }}"
            ...
            <div class="table-responsive">
                <table class="monthly-update-table table table-striped table-bordered">
                    ...
                    <tr>
                        ...
                        <th>@lang('monthly-update.task_name')</th>
                    ...
                    @foreach($monthlyUpdatable->monthlyUpdates->sortByDesc('date') as $monthlyUpdate)
                        <tr>
                            ...
                            <td>
                                @foreach($monthlyUpdatable->tasks as $task)
                                    @if($monthlyUpdate->tasks == $task->id)
                                        {{ $task->name }}
                                    @endif
                                @endforeach
                            </td>
                            ...
                            <td class="text-center">
                                <span class="dropdown">
                                    ...
                                    <span aria-labelledby="btnSearchDrop2"
                                      ...
                                      <a href="{{ route($module . '-monthly-updates.show', [$project->id, $monthlyUpdatable->id, $monthlyUpdate->id])  }}"
                                         class="dropdown-item"><i
                                                  class="ft-eye"></i> {{trans('labels.details')}}</a>
                                      <a href="{{ route($module . '-monthly-updates.edit', [$project->id, $monthlyUpdatable->id, $monthlyUpdate->id])  }}"
                                         class="dropdown-item"><i
