...
class ComplaintInvitation extends Model
{
    use Statable;

    protected $fillable = ['complainer_id', 'complainee_id', 'creator_id', 'reviewer_id', 'reason', 'status'];

    protected function getGraph()
    {
        return 'complaint_invitation';
    }
    ...
    {
        $this->hasOne(Complaint::class, 'complaint_invitation_id', 'id');
    }
    ...
    public function stateDetails()
    {
        return $this->hasManyThrough(StateDetail::class, StateHistory::class, 'statable_id', 'state_history_id', 'id', 'id')
            ->where('statable_type', ComplaintInvitation::class);
    }
