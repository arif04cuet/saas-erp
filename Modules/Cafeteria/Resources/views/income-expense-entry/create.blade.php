            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-content collapse show">
                        <div class="card-body">
                            @include('cafeteria::income-expense-entry.form')
                        </div>
                    </div>
                </div>
            </div>
...
@push('page-js')
    ...
    <!-- custom -->
    <script src="{{ asset('js/cafeteria-accounts/journal-entry.js') }}"></script>
    <script>
        const placeholder = @json(trans('labels.select'));
        $(document).ready(function () {
            initAllRepeater('.cafeteria-journal-entry-repeater');
            validateForm('.cafeteria-journal-entry-form');
        ...
        function expenseLimit(name) {
            ...
            $.ajax({
                url: '{{url('accounts/journals/get-expense-limit')}}/' + $code.val() + '/0/' + $('#date').val(),
                type: 'get',
                dataType: 'text',
