        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">@lang('cafeteria::deliver-material.title') @lang('cafeteria::cafeteria.list')</h4>
                    <a class="heading-elements-toggle"><i class="la la-ellipsis-h font-medium-3"></i></a>
                    @if (!Auth::user()->hasRole(Config::get('constants.cafeteria.roles.cafeteria_manager')))
                        <div class="heading-elements">
                            <a href="{{route('deliver-materials.create')}}" class="btn btn-primary btn-sm"><i
                                    class="ft-plus white"></i> {{ trans('cafeteria::deliver-material.create') }}
                            </a>
                        </div>
                    @endif
                </div>
                            ...
                            <table class="table table-striped table-bordered purchase-list-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th>{{trans('cafeteria::deliver-material.department.title')}}</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            {{ trans('cafeteria::deliver-material.department.' . $item->department) }}
                                        </td>
                                        <td>
                                            ...
                                            <span aria-labelledby="btnSearchDrop2"
                                                ...
                                                @if($item->user_id == Auth::user()->id && Auth::user()->hasRole(Config::get('constants.cafeteria.roles.cafeteria_user')))
                                                    @if ($item->status == "draft")
                                                        <a href="{{ route('deliver-materials.edit', $item->id) }}"
                                                                class="dropdown-item"><i class="ft-edit"></i> {{trans('labels.edit')}}</a>
                                                    @endif
                                                @endif
                                                @if(Auth::user()->hasRole(Config::get('constants.cafeteria.roles.cafeteria_manager')))
                                                    @if($item->status == "pending")
                                                        <a href="{{ route('deliver-materials.approval', $item->id) }}"
                                                            class="dropdown-item"><i class="ft-check-square"></i> {{trans('labels.approve')}}</a>
                                                    @endif
                                                @endif
                                                <a href="{{ route('deliver-materials.show', $item->id) }}"
