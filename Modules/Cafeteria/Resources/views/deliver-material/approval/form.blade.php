<div class="card-body">
    {!! Form::open(['route' => ['deliver-materials.approval', $deliverItem->id ], 'class' => 'form deliver-approval-form']) !!}
    @method('PUT')
    <div id="invoice-items-details" class="">
        <h4 class="form-section"><i class="la la-tag"></i>@lang('cafeteria::deliver-material.title') @lang('cafeteria::cafeteria.information')</h4>
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    {!! Form::label('title', trans('labels.title'), ['class' => 'form-label required']) !!}
                    {!! Form::text('title', $deliverItem->title , ['class' =>
                    "form-control",
                    "required",
                    'placeholder' => trans('labels.title'),
                    'data-msg-required'=> __('labels.This field is required'),
                    'data-rule-maxlength' => 50,
                    'data-msg-maxlength'=> trans('labels.At most 50 characters')
                    ])!!}
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group">
                    {!! Form::label('deliver date', trans('labels.date'), ['class' => 'form-label required']) !!}
                    {!! Form::text('deliver_date', $deliverItem->deliver_date, ['class' =>
                    'form-control',
                    'required',
                    ])!!}
                </div>
            </div>
              <!-- Type -->
            <div class="col-md-6">
                {!! Form::label('type',
                trans('cafeteria::deliver-material.department.title'),
                ['class' => 'form-label required']) !!}
                <p></p>
                <div class="form-check-inline skin skin-flat">
                    {!! Form::radio('department', 'cooking',
                    $deliverItem->department == "cooking" ? true : false,
                    ['class' => 'required']) !!}
                    <label>@lang('cafeteria::deliver-material.department.cooking')</label>
                </div>
                <div class="form-check-inline skin skin-flat">
                    {!! Form::radio('department', 'tea',
                    $deliverItem->department == "tea" ? true : false,
                    ['class' => 'required',]) !!}
                    <label>@lang('cafeteria::deliver-material.department.tea')</label>
                </div>
                <div class="form-check-inline skin skin-flat">
                    {!! Form::radio('department', 'bakery',
                    $deliverItem->department == "bakery" ? true : false,
                    ['class' => 'required',]) !!}
                    <label>@lang('cafeteria::deliver-material.department.bakery')</label>
                </div>
                <div class="form-check-inline skin skin-flat">
                    {!! Form::radio('department', 'snacks',
                    $deliverItem->department == "snacks" ? true : false,
                    ['class' => 'required',]) !!}
                    <label>@lang('cafeteria::deliver-material.department.snacks')</label>
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group">
                    {!! Form::label('remark', trans('labels.remarks'), ['class' => 'form-label']) !!}
                    {!! Form::textarea('remark', $deliverItem->remark, ['class' =>
                    'form-control',
                    'readOnly',
                    'rows' => 2,
                    ])!!}
                </div>
            </div>
            <div>
                {!! Form::hidden('deliver_material_id', $deliverItem->id )!!}
            </div>
        </div>

        <h4 class="form-section"><i class="la la-tag"></i>@lang('cafeteria::purchase-list.details')</h4>
        <div class="row">
            <div class="table-responsive col-sm-12">
                <table class="table table-bordered text-center deliver-material-items">
                    <thead>
                        <tr>
                            <th>{!! Form::checkbox(null, null, false, ['id' => 'checkAll']) !!}</th>
                            <th>@lang('cafeteria::purchase-list.name')</th>
                            <th>@lang('cafeteria::purchase-list.quantity')</th>
                            <th width="20%">@lang('cafeteria::purchase-list.unit')</th>
                            <th width="1%">
                                <i data-repeater-create class="la la-plus-circle text-info" style="cursor: pointer"
                                    id="repeater_create"></i>
                            </th>
                        </tr>
                    </thead>
                    <tbody data-repeater-list="deliver-material-entries">
                        @foreach ($deliverItem->deliverItemLists as $item)
                        <tr data-repeater-item>
                            {{ Form::hidden('item_id', $item->id) }}
                            <td>
                                {!! Form::checkbox('status', null, null, ['class' => 'approvalCheck']) !!}
                            </td>
                            <td width="25%">
                                {!! Form::select('raw_material_id', $rawMaterials, $item->raw_material_id,
                                ['class' =>
                                "form-control material-dropdown-select",
                                "required",
                                'onChange' => 'getUnitByMaterial(this.name)',
                                ])!!}
                            </td>

                            <td> {!! Form::number('quantity', $item->quantity, ['class' =>
                                'form-control spin',
                                'required',
                                'onkeyup' => 'calculateTotal(this.name)',
                                'placeholder' => trans('cafeteria::purchase-list.quantity'),
                                'data-msg-max'=> trans('cafeteria::sales.quantity_validate'),
                                'min' => 1,
                                'data-msg-min'=> trans('labels.Please enter a value greater than or equal to 1'),
                                ])!!}
                            </td>
                            @php
                                $unitName =  app()->isLocale('en') ? $item->rawMaterial->unit->en_name : $item->rawMaterial->unit->bn_name;
                            @endphp
                            <td class="unit"> {!! Form::select('unit_id', [$item->rawMaterial->unit->id => $unitName], null, ['class' => '
                                form-control unit-dropdown-select',
                                'required'
                                ])!!}
                            </td>

                            <td class="support-td"></td>

                            <td class="delBtn" style="display: none"><i data-repeater-delete class="la la-trash-o text-danger"
                                style="cursor: pointer"></i>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
            </div>
        </div>
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    {!! Form::label('approval_note', trans('cafeteria::purchase-list.approval_note'), ['class' =>
                    'form-label']) !!}
                    {!! Form::textarea('approval_note', null, ['class' =>
                    'form-control',
                    'rows' => 2,
                    'data-rule-maxlength' => 255,
                    'data-msg-maxlength'=> trans('labels.At most 255 characters'),
                    ])!!}
                </div>
            </div>
        </div>
    </div>

    <!-- Save & Cancel Button -->
    <div class="form-actions text-center">
        <button type="submit" class="btn btn-success approveBtn" name="status" value="approved">
            <i class="la la-check-square-o"></i>
            @lang('labels.approve')
        </button>
        <button type="submit" class="btn btn-danger" name="status" value="rejected">
            <i class="la la-check-square-o"></i>
            @lang('labels.reject')
        </button>
        <a class="btn btn-warning mr-1" role="button" href="{{route('deliver-materials.index')}}">
            <i class="ft-x"></i> @lang('labels.cancel')
        </a>
    </div>
    {!! Form::close() !!}
</div>
@push('page-js')
<script>
        $("#checkAll").click(function(){
            $('input:checkbox').not(this).prop('checked', this.checked);
            toogleApprovalBtn(this);
        });

        $('.approvalCheck').click(function() {
            toogleApprovalBtn(this);
        });

        function toogleApprovalBtn(data) {
            if ($(data).is(':checked')) {
                $('.approveBtn').prop("disabled", false);
            } else {
                if ($('.approvalCheck').filter(':checked').length < 1){
                    $('.approveBtn').attr('disabled',true);}
            }
        }

        function getUnitByMaterial(name) {
            $index = name.match(/\d+/).toString();
            $rawMateial = $("select[name='deliver-material-entries[" + $index + "][raw_material_id]']");
            $unit = $("select[name='deliver-material-entries[" + $index + "][unit_id]']");
            $quantity = $("input[name='deliver-material-entries[" + $index + "][quantity]']");

            let url = "{{ url('cafeteria/get-unit-by-material/') }}" ;
            $.get( url +'/'+ $rawMateial.val(), function (data) {
                $($unit).find('option').remove();
                $($unit).append(`<option value="${data.unit_id}">${data.unit_name}</option>`)

                $($quantity).attr('max', data.available_amount);
            });
        }


</script>
@endpush
