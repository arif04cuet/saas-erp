                                    ...
                                    <tbody>
                                        @foreach($inventoryRequests as $inventoryRequest)
                                            <tr>
                                                ...
                                                <td>
                                                <span class="dropdown">
                                                    <button id="imsRequestList" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" class="btn btn-info dropdown-toggle">
                                                        <i class="la la-cog"></i>
                                                    </button>
                                                    <span aria-labelledby="imsRequestList" class="dropdown-menu mt-1 dropdown-menu-right">
                                                        <a href="{{ route('inventory-request.show', $inventoryRequest->id) }}" class="dropdown-item"><i class="ft-eye"></i> @lang('labels.details')</a>
                                                        @if($inventoryRequest->status == 'new')
                                                            <a href="{{ route('inventory-request.create.detail', [$inventoryRequest->type, $inventoryRequest->id]) }}" class="dropdown-item"><i class="ft-plus-circle"></i> @lang('labels.add') @lang('labels.details')</a>
                                                        @elseif($inventoryRequest->isRecipient())
                                                            <a href="{{ route('inventory-request.create.detail', [$inventoryRequest->type, $inventoryRequest->id]) }}" class="dropdown-item"><i class="ft-plus-circle"></i> @lang('labels.add') @lang('labels.details')</a>
                                                        @endif
                                                        <div class="dropdown-divider"></div>
                                                        @if(!in_array($inventoryRequest->status, ['approved', 'received', 'rejected']))
                                                            <a href="{{ route('inventory-request.edit.initial', [$inventoryRequest->type, $inventoryRequest->id]) }}" class="dropdown-item"><i class="ft-edit-2"></i> @lang('labels.edit')</a>
                                                        @endif
{{--                                                        <div class="dropdown-divider"></div>--}}
{{--                                                        {!! Form::open([--}}
{{--                                                            'method'=>'DELETE',--}}
