                    ...
                    <div class="card">
                        ...
                        <div class="card-content collapse show">
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-6">
                                        {!! Form::open(['route' =>  ['training-head.store'], 'class' => 'form wizard-circle training-head-form', 'novalidate', 'method' => 'post']) !!}
                                        <fieldset>
                                            @include('tms::training-head.form',['page'=>'index'])
                                        </fieldset>
                                        {!! Form::close() !!}
                                    </div>
                                    <div class="col-6">
                                        <div class="table-responsive">
                                            <table class="master table table-striped table-bordered alt-pagination">
                                                ...
                                                <tbody>
                                                @foreach($trainingHeads as $key => $trainingHead)
                                                    <tr>
                                                        <td scope="row">{{ $loop->iteration }}</td>
                                                        <td>
                                                            <a href="{{ route('training-head.show', $trainingHead->id) }}">
                                                                {{ $trainingHead->getTitle() ?? trans('labels.not_found') }}
                                                            </a>
                                                        </td>
                                                        <td class="text-center">
                                                            <div class="btn-group">
                                                                <a href="{{ route('training-head.show', $trainingHead) }}" class="master btn btn-info" title="{{trans('labels.details')}}">
                                                                    <i class="ft-eye white"></i>
                                                                </a>
                                                                <a href="{{ route('training-head.edit',  $trainingHead) }}" class="master btn btn-success" title="{{trans('labels.edit')}}">
                                                                ...
                                                                {!! Form::open([
                                                                    'method' => 'DELETE',
                                                                    'url' => route('training-name.delete',$trainingHead->id),
...
@push('page-js')
    ...
    <script>
        $(document).ready(function () {
            validateForm('.training-head-form');
        });
    </script>
@endpush
